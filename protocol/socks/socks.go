...
func WriteAddr(w io.Writer, addr Addr) error {
	_, err := w.Write(addr[:1]) // write 1st byte for address type
	if err != nil {
		return err
	}
	switch addr[0] {
	case AtypDomainName:
		_, err := w.Write(addr[1:2]) // write 2nd byte for address length
		if err != nil {
			return err
		}
		_, err = w.Write(addr[2 : 2+int(addr[1])+2])
		return err
	case AtypIPv4:
		_, err = w.Write(addr[1 : 1+net.IPv4len+2])
		return err
	case AtypIPv6:
		_, err = w.Write(addr[1 : 1+net.IPv6len+2])
		return err
	}
...
func isDomainName(host string) bool {
	return !isIPAddress(host) && strings.Contains(host, ".")
}
